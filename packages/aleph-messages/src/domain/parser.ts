...
import {
  SOLANA_MESSAGES_PROGRAM_IDL,
  SOLANA_MESSAGES_PROGRAM_ID_PK,
} from '../constants.js'

export class EventParser {
  protected anchorEventParser = new AnchorEventParser(
    SOLANA_MESSAGES_PROGRAM_ID_PK,
    new BorshCoder(SOLANA_MESSAGES_PROGRAM_IDL),
  )
  ...
  parseSolanaMessages(
    ...
    for (const entity of entities) {
      if (entity.parentTransaction.meta?.logMessages) {
        const logs = this.anchorEventParser.parseLogs(
          entity.parentTransaction.meta?.logMessages,
        ) as unknown as SolanaEvents[]
